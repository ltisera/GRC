...
class ComentarioDAO(ConexionBD):
    ...
    def comentarReferencia(self, comentario, idReferencia, fecha, idUsuario):
        self.crearConexion()
        try:
            if (self._bd.is_connected()):
                consulta = 'INSERT INTO comentario(`comentario`, `fechaHora`, `idReferencia`, `idUsuario`) values("{0}","{1}","{2}","{3}")'.format(comentario, fecha, idReferencia, idUsuario)
